<form id="formulario_usuario" action="<?php $_SERVER['PHP_SELF'] ?>" method="POST">
	<table>
		...
		<tbody>
			<tr>
				<td><h2>Nueva casa de acogida</h2></td>
			</tr>
				<tr>
	...
	<?php
		if (isset($_POST['insertar']))
		{
			...
			$sql = "INSERT INTO `shelter`.`casa_de_acogida` (`DNI`, `nombre`, `correo`, `direccion`, `telefono`) VALUES ('".$_POST['dni_user']."', '".$_POST['nombre_user']."', '".$_POST['email_user']."', '".$_POST['direccion_user']."', '".$_POST['telefono_user']."');";

			$res = mysqli_query($conexion_bd, $sql);

			if($res == 1)
			{
                            header('Location: index.php?t=casa_acogida');
			}else
			{
